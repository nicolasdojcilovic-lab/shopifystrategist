/**
 * Script Simple — Test Playwright Service (Page Simple)
 * 
 * Teste une capture mobile sur example.com.
 * 
 * Usage:
...
async function main() {
  console.log('🚀 Test Playwright Service (Page Simple)\n');

  // URL de test (simple et rapide)
  ...
  try {
    ...
    const result = await service.capturePage(url, 'mobile', {
      timeout: 10000, // 10s devrait suffire pour example.com
    });
    ...
    if (result.success) {
      ...
      const screenshotPath = join(tempDir, 'example-mobile.png');
      ...
      console.log(`\n💡 Le service fonctionne! Vous pouvez maintenant tester avec Gymshark en augmentant le timeout.`);
    } else {
      console.error(`\n❌ Échec de la capture:`);
